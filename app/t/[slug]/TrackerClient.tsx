...
function HistoryModal({ history, onClose }: { history: Entry[]; onClose: () => void }) {
  // Group by date
        ...
        {dates.length === 0 ? (
          ...
          <div className="space-y-2">
            {dates.map((d) => {
              ...
              const food = dayEntries.filter((e) => e.kind === 'food').reduce((s, e) => s + e.points, 0);
              const ex = Math.min(dayEntries.filter((e) => e.kind === 'exercise').reduce((s, e) => s + e.points, 0), 4);
              const net = food - ex;
              return (
                <div key={d} className="flex justify-between items-center py-3" style={{ borderBottom: '1px dotted #c9bfa9' }}>
                  <div>
                    ...
                    <div className="text-xs italic" style={{ color: '#8a7a5c' }}>
                      {dayEntries.filter(e => e.kind === 'food').length} items · {dayEntries.filter(e => e.kind === 'exercise').length} activities
                    </div>
                  </div>
                  <div className="text-xl num" style={{ fontWeight: 500 }}>{net}</div>
                </div>
              );
            })}
          </div>
